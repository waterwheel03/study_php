...
<header>
  <nav class="navbar navbar-default">
    ...
    <div class="navbar-header"><a class="navbar-brand" href="#">写真アップロード</a></div>
      ...
      <li class="next"><a href="data_select.php">画像一覧 →</a></li>
      </ul>
  </nav>
</header>
<!-- Head[End] -->
...
//ファイルアップロードパラメータ取得
//if (!isset($_FILES['upfile']['error']) || !isset($_FILES['upfile']['error'])) {
	// echo 'パラメータが不正です';
//}else{
...
<html lang="ja">
	<head>
		<meta charset="UTF-8">
		<title>CSRFサンプル</title>
	</head>
	<body>
		<h1>お問い合わせフォーム</h1>
		<form action="sample02.php" method="POST">
			<p><label>名前<input type="text" name="name" size="30" value=""></label></p>
			<p><label>電話番号<input type="text" name="tel" size="20" value=""></label></p>
			<input type="hidden" name="token" value="<?php echo htmlspecialchars(session_id(), ENT_QUOTES, 'UTF-8') ?>"></label></p>
